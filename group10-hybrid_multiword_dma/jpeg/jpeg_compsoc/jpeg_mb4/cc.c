...
#ifdef USEDMA
				offset = (3 + 1) * (i_times_MCU_sx + j); //add 1 for the alpha channel
				CB->data[offset + 3] = 0xFF;
#endif // USEDMA
#ifndef USEDMA
...
#ifdef USEDMA
		for (int i = 0; i < (SH1->MCU_sy); i++) {
			for (int j = 0; j < SH1->MCU_sx; j++) {
				int i_times_MCU_sx = i * SH1->MCU_sx;
				int offset = (3 + 1) * (i_times_MCU_sx + j); //add 1 for the alpha channel
				CB->data[offset + 3] = 0xFF;
				CB->data[offset + 2] = PB->linear[i_times_MCU_sx + j];
				CB->data[offset + 1] = PB->linear[i_times_MCU_sx + j];
				CB->data[offset + 0] = PB->linear[i_times_MCU_sx + j];
				/* note that this is SunRaster color ordering */
			}
		}
#endif // USEDMA
