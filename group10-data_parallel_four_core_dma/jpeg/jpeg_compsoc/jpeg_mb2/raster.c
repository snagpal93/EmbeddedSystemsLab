...
//            mk_mon_debug_info(pix->r);
  //          mk_mon_debug_info(pix->g);
    //        mk_mon_debug_info(pix->b);
            #ifdef USEDMA
            pix->a = 0xFF ;
            hw_dma_send( &mem[row_offset + col_offset].pixel , dmamem, 1, &dma, BLOCKING);
            #endif
        }
    }
}
